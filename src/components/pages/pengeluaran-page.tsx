...
export default function PengeluaranPage() {
  ...
  const [deletingItem, setDeletingItem] = useState<Pengeluaran | null>(null)
  ...
  // Fetch data
  const { data: pengeluaranData, isLoading } = useQuery({
    queryKey: ['pengeluaran', filterJenis, filterStartDate, filterEndDate],
    queryFn: async () => {
  ...
  const openEditDialog = (item: Pengeluaran) => {
    setEditingItem(item)
  ...
  const handleDelete = (item: Pengeluaran) => {
  ...
  const totalNominal = pengeluaranList.reduce((sum, p) => sum + p.nominal, 0)
  const totalPages = Math.ceil(pengeluaranList.length / ITEMS_PER_PAGE)
  const paginatedData = pengeluaranList.slice(
  ...
  return (
    <div className="space-y-4">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              <p className="text-sm text-muted-foreground">
                Halaman {currentPage} dari {totalPages} ({pengeluaranList.length} data)
              </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { resetForm(); setEditingItem(null) } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit Pengeluaran' : 'Tambah Pengeluaran'}</DialogTitle>
            <DialogDescription>
              {editingItem ? 'Perbarui data pengeluaran' : 'Tambahkan data pengeluaran baru'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {/* Jenis */}
